...
const navSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/survival-sheet", label: "Final Review", icon: Brain },
    ],
  },
];
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[270px_minmax(0,1fr)]">
      <aside className="sticky top-0 hidden h-screen border-r border-white/10 bg-paper/90 px-4 py-5 shadow-soft backdrop-blur-xl lg:block">
        <NavLink to="/" className="focus-ring flex items-center gap-3 rounded-xl px-2 py-2">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-surface text-ocean shadow-soft">
            <Sigma size={22} />
          </div>
          <div>
            <div className="font-sans text-xl font-semibold leading-tight">Stats Lab</div>
            <div className="text-sm text-graphite">Study and practice</div>
          </div>
        </NavLink>
        ...
        <div className="mt-7 space-y-6">
          {navSections.map((section) => (
            <nav key={section.label}>
              <div className="mb-2 px-2 text-sm font-semibold text-ocean">
                {section.label}
              </div>
        ...
        <div className="absolute bottom-5 left-4 right-4 rounded-2xl border border-white/10 bg-surface/70 p-3 text-sm leading-6 text-graphite shadow-soft">
          <div className="font-semibold text-ink">Study path</div>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-40 border-b border-white/10 bg-paper/90 backdrop-blur-xl lg:hidden">
          ...
          <NavLink to="/" className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-surface text-ocean shadow-soft">
              <Sigma size={22} />
            </div>
            <div>
              <div className="font-sans text-lg font-semibold leading-tight">Stats Lab</div>
              <div className="text-sm text-graphite">Statistics practice</div>
            </div>
          </NavLink>
        </div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `focus-ring flex shrink-0 items-center gap-2 rounded px-3 py-2 text-sm ${
                    isActive ? "bg-lavender text-white shadow-soft" : "bg-surface/80 text-graphite hover:bg-mist"
                  }`
                }
              >
                <Icon size={15} />
